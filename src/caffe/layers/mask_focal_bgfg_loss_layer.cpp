...
#include "caffe/layers/mask_focal_bgfg_loss_layer.hpp"
...
Dtype MaskFocalBGFGLossLayer<Dtype>::calc_grad(Dtype pred, Dtype mask)
{
...
template <typename Dtype>
MaskFocalBGFGLossLayer<Dtype>::MaskFocalBGFGLossLayer(const LayerParameter& param):LossLayer<Dtype>(param)
{
...
template <typename Dtype>
void MaskFocalBGFGLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void MaskFocalBGFGLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void MaskFocalBGFGLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) 
{
    ...
    for (int j = 0; j < num; j++)
    {
        ...
        for (int height = 0; height < predictions_fg->shape(2); height++)
        {
            for (int width = 0; width < predictions_fg->shape(3); width++)
            {
                ...
                Dtype grad_fg = calc_grad(pred_fg,mask);
                Dtype grad_bg = calc_grad(pred_bg,Dtype(1) - mask);
...
#ifdef CPU_ONLY
STUB_GPU(MaskFocalBGFGLossLayer);
#else
NO_GPU_CODE(MaskFocalBGFGLossLayer);
#endif

INSTANTIATE_CLASS(MaskFocalBGFGLossLayer);
REGISTER_LAYER_CLASS(MaskFocalBGFGLoss);
