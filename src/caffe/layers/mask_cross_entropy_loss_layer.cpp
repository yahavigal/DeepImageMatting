...
#include "caffe/layers/mask_cross_entropy_loss_layer.hpp"
...
MaskCrossEntropyLossLayer<Dtype>::MaskCrossEntropyLossLayer(const LayerParameter& param):LossLayer<Dtype>(param)
{
...
template <typename Dtype>
void MaskCrossEntropyLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void MaskCrossEntropyLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void MaskCrossEntropyLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) 
{
    Blob<Dtype>* predictions = bottom[0];
    Blob<Dtype>* masks = bottom[1];
    ...
    for (int j = 0; j < num; j++)
    {
        ...
        for (int height = 0; height < predictions->shape(2); height++)
        {
            for (int width = 0; width < predictions->shape(3); width++)
            {
                ...
                Dtype grad =  (-mask)/(pred + m_epsilon) + (Dtype(1) - mask)/(Dtype(1) - pred + m_epsilon);
